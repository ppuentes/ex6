...
```{r, fig.alt= "After downloading the data, saving it into my project folder, and reading it in, I used the import option to preview the dataset. After doing a quick check, it seemed that there was an extra column and two extra rows on the top. I fixed this using the skip and select arguments. I also changed the weight to numeric variables. By using the mutate argument, I was transformed the character variables into factors, so I could get a better understanding of the count and numbers through the summary argument.}
garden2020 <- read_csv("https://raw.githubusercontent.com/llendway/scraping_etc/main/2020_harvest.csv", 
...
```{r, fig.alt = "After downloading the data, saving it into my project folder, and reading it in, I used the import option to preview the dataset. After doing a quick check, it seemed that most of the data looked good and there were no missing values. By using the mutate argument, I was able to make the character variables into factors, so I could get a better understanding of the count and numbers through the summary argument."}
...
```{r, fig.alt=}
garden_gt <- garden_harvest %>% 
  gt(rowname_col = "row",
    groupname_col = "group") %>% 
  tab_options(column_labels.background.color = "tomato4") %>% 
  fmt_date(columns = vars(date),
           date_style = 6) %>% 
   tab_header(
    title = "2020 Garden Harvest",
    subtitle = md("This data was collected from **Lisa Lendway's** garden in 2020")
  )
...
g1 <- garden_harvest %>% 
  ...
  mutate(wt_lbs = (total_wt/454)) %>%
           mutate(cum_wt = cumsum(wt_lbs)) %>%
  ggplot(aes(x = date, y = cum_wt, color = variety)) +
  geom_line()+
  labs(title = "Cumulative Weight (lbs)") +
...
g2 <- garden_harvest %>% 
  ...
  ggplot() +
  geom_col(aes(x= wt_lbs, y = fct_reorder(variety, wt_lbs, sum)), fill = "tomato3") +
  labs(title = "Total Weight (lbs)") +
...
g3 <- garden_harvest %>% 
  ...
  geom_bar()+
  labs(title = "Number of Beet Harvests") +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())
...
g2 + (g1/g3) + 
  ...
  plot_annotation(title = "Beet Harvests in 2020") 
```
